...
SELECT * FROM student INNER JOIN payment on student.id = payment.student_id INNER JOIN paymenttype ON payment.type_id = paymenttype.id WHERE paymenttype.name = 'WEEKLY' GROUP BY student.id;
